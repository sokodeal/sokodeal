...
export default function Home() {
  ...
  return (
    <>
      ...
      {/* MODAL INSCRIPTION */}
      {signupOpen && (
        <div onClick={() => setSignupOpen(false)} style={{
          position:'fixed', inset:0, background:'rgba(0,0,0,0.6)',
          zIndex:200, display:'flex', alignItems:'center', justifyContent:'center'
        }}>
          <div onClick={e => e.stopPropagation()} style={{
            background:'white', borderRadius:'18px', padding:'32px',
            maxWidth:'480px', width:'100%', margin:'20px'
          }}>
            <h2 style={{fontFamily:'Syne,sans-serif', fontWeight:800, marginBottom:'8px'}}>🦁 Créer un compte</h2>
            <p style={{color:'#6b7c6e', fontSize:'0.88rem', marginBottom:'20px'}}>SokoDeal — Africa's Marketplace</p>
            {['Prénom & Nom', 'Email', 'Téléphone (+250...)', 'Mot de passe'].map(p => (
              <input key={p} type={p.includes('passe') ? 'password' : 'text'} placeholder={p} style={{
